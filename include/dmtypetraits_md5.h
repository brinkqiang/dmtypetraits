...
namespace dm::pack {
    namespace detail {
        ...
        private:
            // 私有构造函数，使用 C++14/17 的 index_sequence 技术在编译期初始化基类
            template <std::size_t... I>
            constexpr string_literal(const CharType(&value)[Size + 1], std::index_sequence<I...>)
                : std::array<CharType, Size + 1>{ value[I]... } // 直接在初始化列表中展开元素
            {
            }
