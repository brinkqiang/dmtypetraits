...
static_assert(std::endian::native == std::endian::little, "only support little endian now");
...
template <typename T>
concept dm_pack_byte = dm_is_same_v<char, T> || dm_is_same_v<unsigned char, T> || dm_is_same_v<std::byte, T>;

template <typename T>
concept dm_pack_buffer = dm_pack_trivially_copyable_container<T> && dm_pack_byte<typename T::value_type>;
...
constexpr auto get_types(U &&t) {
  ...
  if constexpr (dm_is_fundamental_v<T> || dm_is_enum_v<T> || dm_pack_is_std_basic_string_v<T> || 
                ...
                dm_is_optional_v<T> || dm_is_variant_v<T> || dm_pack_expected<T> || 
    ...
    return visit_members(
        std::forward<U>(t), [&]<typename... Args>(Args &&...) DMPACK_CONSTEXPR_INLINE_LAMBDA {
...
consteval type_id get_type_id() {
  ...
  else if constexpr (dm_is_enum_v<U>) { return get_integral_type<dm_underlying_type_t<U>>(); }
  ...
  else if constexpr (dm_pack_string<U>) { return type_id::string_t; }
  ...
  else if constexpr (dm_pack_expected<U>) { return type_id::expected_t; }
...
consteval decltype(auto) get_type_literal_impl(std::index_sequence<I...>);

template <typename Args, std::size_t... I>
consteval decltype(auto) get_variant_literal_impl(std::index_sequence<I...>);
...
consteval decltype(auto) get_type_literal() {
  ...
  if constexpr (id == type_id::monostate_t) {
    if constexpr (dm_pack_expected<ParentArg>) {
...
consteval decltype(auto) check_if_compatible_element_exist_help_coverage(std::index_sequence<I...>) {
...
constexpr std::size_t DMPACK_INLINE calculate_one_size(const T &item) {
  ...
  else if constexpr (dm_is_map_container_v<type> || (dm_is_container_v<type> && !dm_pack_string<type>)) {
    total += sizeof(size_type);
    if constexpr (dm_pack_trivially_copyable_container<type>) { total += item.size() * sizeof(typename type::value_type); }
    else { for (auto &&i : item) { total += calculate_one_size(i); } }
  }
  ...
  else if constexpr (dm_pack_expected<type>) {
    total += sizeof(bool);
    if (item.has_value()) {
...
template <typename T>
concept exist_compatible_member = check_if_compatible_element_exist<decltype(get_types(dm_remove_cvref_t<T>{}))>() == 1;

template <typename T>
concept unexist_compatible_member = check_if_compatible_element_exist<decltype(get_types(dm_remove_cvref_t<T>{}))>() == 0;

template <dm_pack_byte Byte>
class packer {
 public:
 ...
 private:
  constexpr void DMPACK_INLINE serialize_many() { return; }

  constexpr void DMPACK_INLINE serialize_many(const auto &first_item, const auto &...items) {
    serialize_one(first_item);
    serialize_many(items...);
  }

  constexpr void DMPACK_INLINE serialize_one(const auto &item) {
    using type = dm_remove_cvref_t<decltype(item)>;
    static_assert(!dm_is_pointer_v<type>);
    if constexpr (dm_is_monostate_v<type>) { } 
      ...
      if constexpr (dm_pack_trivially_copyable_container<type>) {
      ...
      if constexpr(dm_is_pair_v<type>){
          ...
          std::apply([&](auto &&...items) DMPACK_CONSTEXPR_INLINE_LAMBDA { serialize_many(items...); }, item);
      }
    }
    else if constexpr (dm_is_optional_v<type>) {
      ...
      if constexpr (dm_is_trivially_copyable_v<type>) {
        ...
        visit_members(item, [&](auto &&...items) DMPACK_CONSTEXPR_INLINE_LAMBDA { serialize_many(items...); });
      }
    } else { static_assert(!sizeof(type), "the type is not supported yet"); }
    return;
  }
...
class unpacker {
 public:
  ...
  DMPACK_INLINE
    ...
    if constexpr (dm_is_tuple_v<T>) {
      std::apply(
          [&](auto &&...items) DMPACK_CONSTEXPR_INLINE_LAMBDA {
      ...
      visit_members(t, [&](auto &&...items) DMPACK_CONSTEXPR_INLINE_LAMBDA {
        static_assert(I < sizeof...(items), "out of range");
        code = for_each<I>(field, items...);
      });
    }
 ...
 private:
  ...
  template <bool NotSkip = true>
  constexpr std::errc DMPACK_INLINE deserialize_many(auto &&first_item,
                                                     auto &&...items) {
  ...
  template <bool NotSkip = true>
  constexpr std::errc DMPACK_INLINE deserialize_one(auto &item) {
    std::errc code{};
    using type = dm_remove_cvref_t<decltype(item)>;
    static_assert(!dm_is_pointer_v<type>);
    if constexpr (dm_is_monostate_v<type>) { }
      ...
      if constexpr (dm_is_set_container_v<type>) {
        ...
        if constexpr (dm_pack_trivially_copyable_container<type>) {
          ...
          if constexpr (NotSkip) {
            if constexpr (dm_pack_string_view<type>) { item = {(char*)data_ + pos_, container_size}; }
            else {
        ...
        if constexpr(dm_is_pair_v<type>){
            ...
            std::apply([&](auto &&...items) DMPACK_CONSTEXPR_INLINE_LAMBDA { code = deserialize_many<NotSkip>(items...); }, item);
        }
    }
    else if constexpr (dm_is_optional_v<type>) {
      ...
      if constexpr (dm_is_trivially_copyable_v<type>) {
        ...
        visit_members(item, [&](auto &&...items) DMPACK_CONSTEXPR_INLINE_LAMBDA { code = deserialize_many<NotSkip>(items...); });
      }
    }
    else { static_assert(!sizeof(type), "the type is not supported yet"); }
    return code;
  ...
  DMPACK_INLINE constexpr decltype(auto) for_each(FiledType &field, Args &&...items) {
    bool stop = false;
    std::errc code{};
    [&]<std::size_t... I>(std::index_sequence<I...>) DMPACK_CONSTEXPR_INLINE_LAMBDA {
      ((!stop && (stop = set_value<I, FiledIndex>(code, field, items))), ...);
    } (dm_make_index_sequence<sizeof...(Args)>{});
    return code;
  }
